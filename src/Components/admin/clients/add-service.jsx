...
const AddClientsService = ({ handleSubmit, services }) => {
  const router = useRouter();

  const formik = useFormik({
    initialValues: {
      nameRu: services?.name?.ru || "",
      nameUz: services?.name?.uz || "",
    ...
    onSubmit: async ({ nameRu, nameUz, slug }) => {
      ...
      if (result.status === 200) {
        toast.success(services ? "Услуга обновлена!" : "Услуга добавлена!");
        if (result.id) {
  ...
  return (
    <form onSubmit={formik.handleSubmit} encType="multipart/form-data">
      <Card>
        <CardHeader
          subheader={services ? "Редактировать категорию" : "Добавить категорию"}
          title="Категория"
        />
        <Divider />
        <CardContent>
...
export default AddClientsService;
