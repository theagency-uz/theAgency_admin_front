...
import { getWorkCategories } from "src/services/work";
...
const Category = () => {
        ...
        cell: (row) => {
          const handleDelete = async (e) => {
            ...
            if (check) {
              ...
              const updatedCategories = await getArticleCategories();
          ...
          return (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
              <Button sx={{ padding: "5px" }}>
                <Link href={`/works/category/add-category?id=${row.id}`} passHref>
                  <EditIcon />
                </Link>
              </Button>
  ...
  useEffect(() => {
    async function fetchCategories() {
      const allCategories = await getWorkCategories();
  ...
  async function handleSearch(e) {
    ...
    allCategories.forEach((category) => {
      ...
      } else if (category.name.ru.toString().toLowerCase().includes(value.toLowerCase())) {
        searchedCategories.push(category);
  ...
  async function handleDelete(selectedRows) {
    const result = await deleteBulkArticleCategory([...selectedRows.map((r) => r.id)]);
    const allCategories = await getArticleCategories();
    setCategories(allCategories);
  }
  if (loading) {
      ...
      >
        <Container maxWidth={false}>
          <TableToolbar
            ...
            title={"Категория"}
            link="/works/category/add-category"
          />
